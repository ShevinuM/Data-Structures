...
class Solution {
    public int[][] intervalIntersection(int[][] firstList, int[][] secondList) {
        ...
        while (a < firstList.length && b < secondList.length) {
            if (secondList[b][0] <= firstList[a][1] &&
                    firstList[a][0] <= secondList[b][1]) {
                int val1 = Math.max(firstList[a][0], secondList[b][0]);
                int val2 = Math.min(firstList[a][1], secondList[b][1]);
                res_list.add(new int[] { val1, val2 });
            }
