...
class Solution {
    /*
     * This solution uses a hashmap to map the original node to the new node. Then, we 
     * iterate through the original list and set the next and random pointers of the new
     * nodes to the new nodes that correspond to the original nodes.
     */
    public Node copyRandomList(Node head) {
        ...
        Node headCopy = head;
        while (headCopy != null) {
            clone = new Node(headCopy.val);
            nodeMap.put(headCopy, clone);
            headCopy = headCopy.next;
        }
        headCopy = head;
        while (headCopy != null) {
            clone = nodeMap.get(headCopy);
            clone.next = nodeMap.get(headCopy.next);
            clone.random = nodeMap.get(headCopy.random);
            headCopy = headCopy.next;
        }
